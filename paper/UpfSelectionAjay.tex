...
%         not merely a convergence speed optimisation.
Under standard load (S1, 5.5$\times$), warm-start tracking acts as a
critical stabilisation mechanism that prevents discrete-time replicator
dynamics from collapsing into unstable boundary states; it converges in
...
of~\cite{Alevizaki2021upf}.
% P8-Q4: Footnote upgraded to acknowledge stability implications of
%         the pairwise form and the role of warm-start as a necessary
%         stabilisation mechanism, not merely a convergence speedup.
...
replicator, the Jacobian sign structure of the discrete-time
implementation differs near boundary states. This difference
means warm-start tracking functions as a \emph{critical
stabilisation mechanism} --- not merely a convergence speedup ---
that anchors the state vector in the vicinity of the interior
equilibrium and prevents the discrete-time dynamics from
collapsing into unstable boundary states (such as the all-MEC
boundary observed in cold-start S2 trials). The cold-start
boundary-convergence phenomenon is a direct structural consequence
of this dynamic, not merely a cold-start numerical artifact.}
...
\begin{equation*}
  ...
  = 58{,}855~\text{Mbps}
\end{equation*}
% P8-Q1: Corrected CC delay at S4 Phase II onset from erroneous 27.6 ms
%         to the correct value of 18.5 ms, using the per-slice fractions
%         from Table 3 and Eq. (4), as confirmed by egt_controller.py output.
...
speedup: it prevents discrete-time replicator collapse into
boundary states by anchoring the population state in the
neighbourhood of the interior Nash equilibrium.
