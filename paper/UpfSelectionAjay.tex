...
latency thresholds of AR/VR rendering. 3GPP TR~26.918~\S5.2
defines immersive VR latency as $\leq 20$~ms motion-to-photon
end-to-end, with the network contribution bounded at $\leq 10$~ms.
The 10~ms PDB directly represents this network-layer budget
within the total 20~ms target, leaving headroom for the
rendering and display pipeline. The sub-millisecond targets of
...
throughput ($\ll 1$~Mbps)~\cite{3gpp_ts22261} exerts negligible UPF selection
...
the internal SMF N4 session context database at each
...
The model focuses on the UPF processing and propagation
components of end-to-end delay. Radio access latency (typically
4--5~ms one-way for NR Uu per 3GPP TR~38.913),
GTP-U encapsulation overhead ($\approx 0.1$--$0.2$~ms for
software UPF), and N6 egress delay are omitted as they are
constant across UPF selection choices and do not affect the
relative payoff ordering that drives the EGT equilibrium.
...
the steering logic's stability. Because these parameters were
calibrated at a $\approx 20$~Gbps baseline, applying them at the
71--130~Gbps ranges of S1 and S2 constitutes a mathematical
extrapolation. Consequently, the millisecond delay figures reported
in Section~\ref{sec:results} should be interpreted as ordinal
indicators of extreme congestion relative to the PDB, rather than
exact physical latency predictions on COTS hardware. In a physical
software UPF deployment, extreme overload would result in packet
drops and session rejections rather than unbounded exponential
queuing delay.
...
\begin{figure*}[t]
    ...
    $\mathbf{x}^g$ from its internal session context database, and iterates
...
The EGT controller reads per-UPF session counts from
the SMF's internal N4 session context database to compute
$\mathbf{x}^g(t)$, consistent with the population state
definition in Section~\ref{sec:egt_model}.
...
\begin{table}[ht]
\centering
\caption{Scenario Definition: Phase~I/II/III Parameters and
         Initialisation Conditions}
\label{tab:scenario_def}
\renewcommand{\arraystretch}{1.15}
\setlength{\tabcolsep}{3pt}
\begin{tabular}{p{1.9cm}p{0.75cm}p{1.5cm}p{1.8cm}p{0.8cm}p{0.7cm}}
\toprule
\textbf{Scenario} &
  \textbf{$\lambda$} &
  \textbf{Ph.I} &
...
\midrule
S1 Standard
  & 5.5$\times$ & 0.011$\to$1.0 & Warm        & 0.189 & 0.309 \\
S2 Extreme
  & 10$\times$  & 0.011$\to$1.0 & Warm        & 0.170 & 0.278 \\
S3 MEC-Bias
  & 5.5$\times$ & 0.011$\to$1.0 & Cold ($x\!=\!0.90$) & 0.900 & 0.900 \\
S4 CC-Bias
  & 5.5$\times$ & 0.509$\to$5.0 & Warm (elev.)       & 0.143 & 0.231 \\
\bottomrule
\noalign{\smallskip}
\multicolumn{6}{l}{%
  \parbox{8.3cm}{\footnotesize
    
    Ph.III $\lambda$: S1/S3/S4: $5.0\!\to\!0.3\times$;
    S2: $9.0\!\to\!0.3\times$.
    S3 Ph.II: cold-start $x_\text{mec}\!=\!0.90$,
    overriding Ph.I warm-start.}}\\
\end{tabular}
\end{table}
...
Static 20/80 Baseline
  & 0.200 & 0.200 & 14.3~Gbps & 57.2~Gbps & 0 & 27{,}000 & N/A \\
\bottomrule
...
\begin{figure*}[t]
    \centering
    \includegraphics[width=\textwidth]{multi_scenario_1000ue.png}
...
MEC start) converges in 270~iterations. The slightly elevated
S3 URLLC violation count (26,514 vs S1's 26,316) reflects the
additional MEC-side transient during the 670-iteration cold-start
convergence phase, in which the initial 90\% MEC occupancy temporarily
overloads $\text{UPF}_\text{mec}$ before the replicator redistributes
sessions toward the Nash fraction. S4's faster cold-start
...
347~iterations (verified on current implementation) at an intermediate state
...
($u_\text{mec} \approx 4.06\times10^{-7}$). Although the payoff
\emph{difference} (gap: 0.020) remains above the
$\varepsilon = 0.01$ tolerance, the strategy-change gradient
($|\Delta x| < 2\times 10^{-5}$) falls below the secondary stopping
criterion prematurely. This is a \emph{vanishing-gradient arrest}: the
...
trajectory from Phase~I. Warm-start tracking is an \emph{empirically
demonstrated stabilisation mechanism} --- formally characterised
via Jacobian eigenspectrum analysis in Section~\ref{sec:future} (future work):
by keeping the population state in the
...
of the most naive deployment. A static 20/80 split (anchoring
20\% of traffic at MEC) represents an intelligent capacity-aware
baseline that respects the 10:1 cloud-to-edge ratio. Under this
baseline, eMBB MEC delay reaches 70.6~ms and CC delay 16.9~ms,
The weighted average eMBB delay is therefore
$0.20 \times 70.6 + 0.80 \times 16.9 = 14.1 + 13.5 = 27.6$~ms,
confirming zero eMBB violations (matching the EGT performance).
However, URLLC MEC delay hits
14.5~ms and CC delay 16.9~ms. Because both nodes exceed the
10~ms PDB, the 20/80 static split produces 27,000 URLLC violations
(100\% of Phase~II) --- confirming that no static policy can resolve
the infrastructure ceiling at $5.5\times$ load. Interestingly, the
static 20/80 split yields lower absolute URLLC delays (14.5~ms)
than the EGT Nash equilibrium (19.4~ms). This exposes a
coordination-efficiency gap between the decentralised Nash
equilibrium and a centrally optimised static assignment: the EGT
controller achieves a strictly mathematically stable operating
point, but the static 20/80 baseline achieves lower URLLC delay
at the cost of dynamic adaptability.
Quantifying this gap and optimising for social welfare against the
EGT Nash baseline is left for future work.
...
Under standard load (S1), warm-start tracking accumulates
at most 122 iterations at load-transition boundaries across
135 Phase~II steps (total: 442 iterations, mean 3.3 per step).
Under the extreme 10$\times$ surge (S2), the maximum is
147 iterations at a single isolated step, with 98.5\% of steps
requiring only 1 iteration --- both consistent with the
convergence guarantees of~\cite{Alevizaki2021upf}.
