...
\textbf{S1 Standard} & 0.156 / 0.222 & 12.54 Gbps & 58.96 Gbps & 19.5 ms & 18.3 ms & 0 & 26,316 & 505 / 122 \\
...
\begin{itemize}
    ...
    \item \textbf{Gradual Phase Tracking in S2 (Extreme Spike):} Under the $10\times$ stress test (S2), a static single-step EGT optimization from a cold-start state ($x=0.5$) locks the coarse-$\epsilon$ controller ($\epsilon=0.01$) into a sub-optimal numerical trap at $x_{\text{eMBB\_MEC}} \approx 0.380$ --- producing a catastrophic MEC delay of $\approx 2.4 \times 10^6$ ms. Achieving the true Nash Equilibrium ($x_{\text{eMBB\_MEC}} \approx 0.136$, average delay $\approx 188.8$ ms) from a cold start requires tightening $\epsilon$ to $10^{-5}$, which demands $>7{,}000$ iterations. In our continuous simulation, the EGT controller instead leverages a warm-start state inherited from Phase~I ($x_{\text{eMBB\_MEC}} \approx 0.229$ at $1\times$ load) and tracks the $10\times$ load progressively across 135 Phase~II timesteps. By the final Phase~II step, the controller has migrated to $x_{\text{eMBB\_MEC}} = 0.147$, achieving an average eMBB delay of $189.3$~ms and MEC load of $23.80$~Gbps --- demonstrating that continuous warm-start phase tracking produces substantially better outcomes than any cold-start static optimization.
