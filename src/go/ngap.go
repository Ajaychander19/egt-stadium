...
// UEOverride holds per-UE parameters that override the base UE config.
// MSIN must be unique per UE. SST/SD/DNN select the correct slice.
type UEOverride struct {
...
type GNB struct {
	...
	UEList          []UEOverride `json:"UEList,omitempty"` // per-UE overrides
	...
	Recv struct {
		...
		// Multi-UE lookup maps
		TeidToCamper map[uint32]*Camper
		IpToCamper   map[string]*Camper
	}
...
func NewNGAP(filename string) (p *GNB) {
	...
	p.dbgLevel = 0
	p.TeidToCamper = make(map[uint32]*Camper)
	p.IpToCamper = make(map[string]*Camper)

	return
}

var RanUeNgapId uint32 = 1
var RanUeGtpTeid uint32 = 100
...
func (gnb *GNB) MakePDUSessionResourceSetupResponse(ue *nas.UE) (pdu []byte) {
	...
	tmp = gnb.pdusetupresponseRANUENGAPID(c.RanId)
	v = append(v, tmp...)
...
func (gnb *GNB) MakeInitialContextSetupResponse(ue *nas.UE) (pdu []byte) {
	...
	tmp = gnb.contextRespencRANUENGAPID(c.RanId)
	v = append(v, tmp...)
...
func (gnb *GNB) MakeInitialUEMessage(ue *nas.UE) (pdu []byte) {
	...
	tmp := gnb.encRANUENGAPID(c.RanId)
	v = append(v, tmp...)
...
func (gnb *GNB) decProtocolIE(c *Camper, pdu *[]byte) (c2 *Camper, err error) {
	...
	*/
	...
	case idULNGUUPTNLInformation: // 139
		gnb.decUPTransportLayerInformation(c, pdu, length)
	default:
...
*/
func (gnb *GNB) encUPTransportLayerInformation(c *Camper, pre *per.BitField) (pdu []byte) {
	...
	*pre = bf
	...
	tmp = gnb.encGTPTEID(c)
	pdu = append(pdu, tmp...)

	return
}

func (gnb *GNB) decUPTransportLayerInformation(c *Camper, pdu *[]byte, length int) {
	...
	gnb.decTransportLayerAddress(c, &tli)
	gnb.decGTPTEID(c, &tli.Value)
...
func (gnb *GNB) decTransportLayerAddress(c *Camper, tla *per.BitField) {
...
*/
func (gnb *GNB) encGTPTEID(c *Camper) (pdu []byte) {
	...
	teid := make([]byte, 4)
	gnb.GTPuTEID = RanUeGtpTeid
	if c != nil {
		c.GtpLocalTeid = gnb.GTPuTEID
	}
	RanUeGtpTeid++
...
func (gnb *GNB) decGTPTEID(c *Camper, pdu *[]byte) {

	id := readPduUint32(pdu)
	gnb.dprint("GTP TEID: %d", id)
	if c != nil {
		c.GtpPeerTeid = id
		gnb.TeidToCamper[id] = c
	}
...
func (gnb *GNB) encQosFlowPerTNLInformation(
	...
	tmp := gnb.encUPTransportLayerInformation(c, pre)
	pdu = append(pdu, tmp...)
...
*/
func (gnb *GNB) pdusetupresponseRANUENGAPID(ranId uint32) (v []byte) {

	head, _ := encProtocolIE(idRANUENGAPID, ignore)

	_, v, _ = per.EncInteger(int64(ranId), 0, 4294967295, false)
	bf, _ := per.EncLengthDeterminant(len(v), 0, 0)
	head = append(head, bf.Value...)
	v = append(head, v...)
	return
...
RAN-UE-NGAP-ID ::= INTEGER (0..4294967295)
*/
func (gnb *GNB) encRANUENGAPID(ranId uint32) (v []byte) {
...
RAN-UE-NGAP-ID ::= INTEGER (0..4294967295)
*/
func (gnb *GNB) contextRespencRANUENGAPID(ranId uint32) (v []byte) {
